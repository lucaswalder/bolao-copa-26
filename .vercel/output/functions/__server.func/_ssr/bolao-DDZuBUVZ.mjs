...
//#region node_modules/.nitro/vite/services/ssr/assets/bolao-DDZuBUVZ.js
...
function estimatePreMatchOdds(matchId, homeScore, awayScore, guessRows) {
	const matchGuesses = guessRows.filter((g) => g.matchId === matchId);
	if (matchGuesses.length === 0) return 6;
	const sameGuess = matchGuesses.filter((g) => g.homeScore === homeScore && g.awayScore === awayScore).length;
...
var getBolaoData = createServerFn({ method: "GET" }).handler(getBolaoData_createServerFn_handler, async () => {
	...
	return {
		...
		matches: matchRows.map((match) => {
			...
			const estimatedOdds = !hasResult && userGuess ? estimatePreMatchOdds(match.id, userGuess.homeScore, userGuess.awayScore, guessRows) : null;
