...
//#region node_modules/.nitro/vite/services/ssr/assets/bolao-CnGkVThf.js
...
function computeX1Standings(userRows, challengeRows, matchById, guessRows) {
...
var getX1Data = createServerFn({ method: "GET" }).handler(getX1Data_createServerFn_handler, async () => {
	await ensureSeedMatches();
	const [{ asc }, { db }, { guesses, matches, user, x1Challenges }] = await Promise.all([
		import("../_libs/drizzle-orm.mjs").then((n) => n.t),
		import("./db-BX8pGjWj.mjs").then((n) => n.n).then((n) => n.n),
		import("./schema-DVvrMujE.mjs").then((n) => n.r).then((n) => n.t)
	]);
	...
	const { standings, duels } = computeX1Standings(userRows, challengeRows, new Map(matchRows.map((match) => [match.id, match])), guessRows);
...
var buyCard = createServerFn({ method: "POST" }).validator(object({ rarity: _enum([
	...
	const [{ asc }, { guesses, matches, user, x1Challenges, championPicks }] = await Promise.all([import("../_libs/drizzle-orm.mjs").then((n) => n.t), import("./schema-DVvrMujE.mjs").then((n) => n.r).then((n) => n.t)]);
	...
	const currentPoints = computeStandings(userRows, guessRows, new Map(matchRows.map((m) => [m.id, m])), challengeRows, championPickRows).find((p) => p.id === sessionUser.id)?.points ?? 0;
	if (currentPoints < cost) throw new Error(`Voce precisa de ${cost} pontos para comprar uma carta ${data.rarity}. Voce tem ${currentPoints}.`);
